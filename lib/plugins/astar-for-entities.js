...
ig.Entity.inject({
    ...
    initMalus: function(){
        if(!this.directionChangeMalus45degree){
            this.directionChangeMalus45degree = this.pathMap.chunkSizePx / 4;
        }
        if(!this.directionChangeMalus90degree){
            this.directionChangeMalus90degree = this.pathMap.chunkSizePx * 5 / 8;
        }
    },

	getPath: function(destinationX, destinationY, diagonalMovement, entityTypesArray, ignoreEntityArray, map) {
	...
	_addEraseEntities: function(addErase, entityTypesArray, ignoreEntityArray) {
		...
		for(i = 0; i < entityTypesArray.length; i++) {
			...
			for(j = 0; j < entities.length; j++) {
				...
				if(!ignoreThisEntity) {
					var sizeX = (entities[j].size.x / this.pathMap.chunkSizePx).floor();
					var sizeY = (entities[j].size.y / this.pathMap.chunkSizePx).floor();

					for(k = 0; k < sizeX; k++) {
						for(l = 0; l < sizeY; l++) {
							var changeTileX = (entities[j].pos.x / this.pathMap.chunkSizePx).floor() + k,
								changeTileY = (entities[j].pos.y / this.pathMap.chunkSizePx).floor() + l;
							if(changeTileX >= 0 && changeTileX < this.pathMap.width && changeTileY >= 0 && changeTileY < this.pathMap.height) {
								if(addErase && this.pathMap.data[changeTileY][changeTileX] == 0) {
									this.pathMap.data[changeTileY][changeTileX] = 9999;
								} else if(!addErase && this.pathMap.data[changeTileY][changeTileX] == 9999) {
									this.pathMap.data[changeTileY][changeTileX] = 0;
								}
							}
						}
					}
	...
	followPath: function(speed, alignOnNearestTile) {
		...
		if(!this.path && alignOnNearestTile) {
			...
			var cx = (this.pos.x / this.pathMap.chunkSizePx).floor() * this.pathMap.chunkSizePx,
				cy = (this.pos.y / this.pathMap.chunkSizePx).floor() * this.pathMap.chunkSizePx;
			...
			if(cx != this.pos.x || cy != this.pos.y) {
				...
				var dxp = cx + this.pathMap.chunkSizePx - this.pos.x,
					dyp = cy + this.pathMap.chunkSizePx - this.pos.y;

				// Choose the smaller distance
				if(dx < dxp) {
					var tx = cx;
				} else {
					var tx = cx + this.pathMap.chunkSizePx;
				}

				if(dy < dyp) {
					var ty = cy;
				} else {
					var ty = cy + this.pathMap.chunkSizePx;
				}
	...
	drawPath: function(r, g, b, a, lineWidth) {
		if(this.path) {
			var mapTilesize = this.pathMap.chunkSizePx;
