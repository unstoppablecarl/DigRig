...
.defines(function(){
    ...
    MyGame = ig.Game.extend({
        ...
        initPatterns:function(){
            ...
            this.removableColor = this.bgContext.createPattern(this.patterns[0], 'repeat');
            this.fixedColor = this.bgContext.createPattern(this.patterns[1], 'repeat');
        ...
        initDynamicBackground:function(){
            ...
            this.addItem({
                ...
                draw: (function(){
                    ...
                    this.font.draw( 'MOVE: Arrow Keys / WASD \nJUMP: space / up / W\nSHOOT DESTROY: click, hold to charge\nSHOOT CREATE: right click, hold to charge\n', 2, 2 );

                if(this.player){
                   this.font.draw( 'Material: ' + this.player.material, 5, 450 );
                   this.font.draw( 'Charge: ' + this.player.charge, 5, 460 );
...
//                                )
        ...
        loadLevel:function(level){
            ...
            for(var i = 0; i < level.entities.length; i++) {
                    ...
                    level.entities[i].x *=2;
                    level.entities[i].y *=2;
            }

            // load collision and fixed layers from images
            for(i = 0; i < level.layer.length; i++) {
                if(
                    level.layer[i].name === '_collision' || level.layer[i].name === 'fixed'
                ){
        ...
        changeLevel: function(levelName){
			...
			ig
				...
				.requires(levelPath)
				.defines(function(){
                    // set levelToLoad on MyGame so when a new instance is created it loads the new level
                    MyGame.prototype.levelToLoad = ig.global[levelObjName];
                    var loader = new ig.Loader( MyGame, ig.resources );
                    loader.load();
